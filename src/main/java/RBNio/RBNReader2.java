...
public class RBNReader2{
	...
	//		RBN parsedrbn = new RBN(probrelsdefined.size());
		...
		RBN result = new RBN(probrelsdefined.size(), 0);
		...
		while (e.hasMoreElements()){
			...
			result.insertPRel(new RBNPreldef((BoolRel)nextprdef.rel(), StringOps.stringVectorToArray(nextprdef.args()), nextprdef.pf()), index);
			index++;
		}
		return result;
	...
	protected void setMacroPF(Rel r, ProbForm pf){
		...
		prdef.setPF(pf);
		pf.setAlias(new ProbFormAtom(r,pf.freevars()));
	}
