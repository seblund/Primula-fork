...
//       return new ProbFormIndicator(rel, relation.getArgumentsArray());
    ...
    public void fileFinished() {
        network = new RBN(probabilisticRelations.size(), formulas.size());
        ...
        for (ProbForm formula : formulas) {
            ...
            network.insertMacro(new RBNMacro(relation, formula.freevars(), formula), currentForm);
        ...
        for (BoolRel rel: rels){
            if(probabilisticRelations.contains(rel.printname())){
                ...
                network.insertPRel(new RBNPreldef(rel, args, new ProbFormConstant(0.5)), currentForm);
